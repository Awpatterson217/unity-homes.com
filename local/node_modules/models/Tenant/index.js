...
const RegisteredTenant = function() {
  ...
  this.authenticate = async function(email, password, callback) {
    ...
    if (!thisPassword.safe)
      ...
      try{
        const user = await _find('registeredUsers', {'email': thisEmail.val});
  ...
  this.create = function(callback) {
    ...
    _count('tenant', {
      ...
      if (!count) {
        _create('tenant', dataObj, function(error, user) {
          if (error !== null)
            return callback(newErr(error));
  ...
  this.delete = function(filter, callback) {
    _delete('tenant', filter, function(error, numOfDeletes) {
      if (error !== null)
        return callback(newErr(error));
  ...
  this.all = async function() {
    try {
      const tenants = await _all('tenant');
  ...
  this.find = async function(filter) {
    ...
    try {
      const tenant = await _find('tenant', filter);
